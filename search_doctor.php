...
<div class="container">
  ...
  <div class="doctor-list m-0" id="doctorList">
    ...
    $query = isset($_SESSION['doctor_id']) 
      ? "SELECT * FROM doctor_register WHERE id = $doctorid" 
      : "SELECT * FROM doctor_register";
    ...
    while ($row = $result->fetch_assoc()) {
      ...
      <div class='doctor-card' 
        ...
        data-specialization='".strtolower($row['category'])."' 
        data-timing='morning evening night' 
        style='display: flex; flex-wrap: wrap; gap: 30px; align-items: center; border: 1px solid #ccc; padding: 15px; margin-bottom: 15px;'>
        <div class='card_left' style='flex: 1 1 300px; display: flex; gap: 20px; align-items: center;'>
          <img src='Doctor_dashboard/src/html/imagedata/".$row['profile_pic']."' alt='Doctor Image' style='width: 100px; height: 100px; object-fit: cover; border-radius: 8px;'>
...
<script>
  ...
  function filterDoctors() {
    ...
    const selectedTiming = timingFilter.value.toLowerCase();
    ...
    doctorCards.forEach(card => {
      ...
      const matchSpecialization = selectedSpecialization === '' || specialization === selectedSpecialization;
      const matchTiming = selectedTiming === '' || timing.includes(selectedTiming);

      if (matchName && matchCity && matchSpecialization && matchTiming) {
  ...
  timingFilter.addEventListener('change', filterDoctors);
</script>
